...
<?php 
	...
	while($item = $res->fetchArray(SQLITE3_ASSOC) ){	 
	    if($item['ISPASS']==1){
	    echo  "	<div class='item'> 
		<form name='consult'  method='post' action='http://localhost/healthyOne/controller/activityManageProcess.php'>
			<p class='name'><b>".$item['USERNAME']."  (".$item['TYPE'].")"."</b></p>
			<p class='profile'>".$item['INTRODUCTION']."</p>";
           echo "</form>";
           echo "<a href='http://localhost/healthyOne/view/consult_question.php?id=".$item['USERNAME']."' class='consult_submit' id='consult'>咨询</a>";
           echo "</div>";
	    }
	}
	?>
